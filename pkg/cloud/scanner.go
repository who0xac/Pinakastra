...
func (s *Scanner) ScanAll(ctx context.Context) []Asset {
	...
	// Scan each provider (sequential to avoid rate limits and crashes)
	for _, name := range bucketNames {
		...
		// 1. AWS S3
		assets := s.checkAWSS3(ctx, name)
		allAssets = append(allAssets, assets...)

		// 2. Google Cloud Storage
		assets = s.checkGCS(ctx, name)
		allAssets = append(allAssets, assets...)

		// 3. Azure Blob Storage
		assets = s.checkAzureBlob(ctx, name)
		allAssets = append(allAssets, assets...)

		// 4. DigitalOcean Spaces
		assets = s.checkDOSpaces(ctx, name)
		allAssets = append(allAssets, assets...)

		// 5. Wasabi
		assets = s.checkWasabi(ctx, name)
		allAssets = append(allAssets, assets...)

		// 6. Backblaze B2
		assets = s.checkBackblazeB2(ctx, name)
		allAssets = append(allAssets, assets...)

		// 7. Alibaba Cloud OSS
		assets = s.checkAlibabaOSS(ctx, name)
		allAssets = append(allAssets, assets...)

		// 8. Firebase Storage
		assets = s.checkFirebase(ctx, name)
		allAssets = append(allAssets, assets...)
