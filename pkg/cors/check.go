...
// CheckSubdomain checks for CORS misconfigurations on a subdomain
func (c *Checker) CheckSubdomain(ctx context.Context, subdomain string) []Issue {
	...
	for _, baseURL := range urls {
		for _, origin := range testOrigins {
			...
			issue := c.testOrigin(ctx, baseURL, origin, subdomain)
			if issue != nil {
		...
		if len(issues) > 0 {
			break // Found issues, no need to test HTTP if HTTPS worked
		}
	}
