...
// ScanSubdomain scans a subdomain for secrets in JS files
...
func (s *Scanner) ScanSubdomain(ctx context.Context, subdomain string) []Finding {
	...
	for _, jsURL := range jsFiles {
		...
		findings := s.scanJSFile(ctx, jsURL)
...
// sanitizeMatch sanitizes sensitive data in match (partially redact)
func sanitizeMatch(match string) string {
	// Don't fully expose secrets in output, partially redact
	if len(match) > 20 {
