...
func cleanAndMergeSubdomains(outputDir, domain string) ([]string, error) {
	...
	// Special handling for amass (remove ANSI codes)
	amassFile := filepath.Join(outputDir, "amass.txt")
	if fileExists(amassFile) {
		content, err := os.ReadFile(amassFile)
		if err == nil {
			// Remove ANSI escape codes
			ansiRegex := regexp.MustCompile(`\x1B\[[0-9;]*[mK]`)
			cleanedContent := ansiRegex.ReplaceAllString(string(content), "")

			// Extract subdomains matching domain pattern
			domainPattern := regexp.MustCompile(fmt.Sprintf(`([a-zA-Z0-9_-]+\.)+%s`, regexp.QuoteMeta(domain)))
			matches := domainPattern.FindAllString(cleanedContent, -1)

			for _, match := range matches {
				cleaned := cleanSubdomain(match, domain)
				if cleaned != "" {
					allSubdomains = append(allSubdomains, cleaned)
				}
			}
