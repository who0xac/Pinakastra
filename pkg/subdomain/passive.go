...
// Run executes all subdomain enumeration tools in parallel
func (p *PassiveEnumerator) Run(ctx context.Context) ([]string, error) {
	...
	// Add Puredns unless explicitly skipped
	if !p.SkipPuredns {
		tools = append(tools, p.runPuredns)
	}

	// Launch all tools in parallel
	for _, tool := range tools {
		wg.Add(1)
		go func(toolFunc func(context.Context) ToolResult) {
			defer wg.Done()
			result := toolFunc(ctx)
			p.Results <- result
		}(tool)
	...
	// Close results channel when all tools complete
	go func() {
		wg.Wait()
		close(p.Results)
	}()
